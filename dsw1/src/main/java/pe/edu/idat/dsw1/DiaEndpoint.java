package pe.edu.idat.dsw1;


import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.ws.server.endpoint.annotation.Endpoint;
import org.springframework.ws.server.endpoint.annotation.PayloadRoot;
import org.springframework.ws.server.endpoint.annotation.RequestPayload;
import org.springframework.ws.server.endpoint.annotation.ResponsePayload;

import io.spring.guides.gs_producing_web_service.GetDiaRequest;
import io.spring.guides.gs_producing_web_service.GetDiaResponse;

@Endpoint
public class DiaEndpoint {
	private static final String NAMESPACE_URI = "http://spring.io/guides/gs-producing-web-service";

	private DiaRepository diaRepository;

	@Autowired
	public DiaEndpoint(DiaRepository diaRepository) {
		this.diaRepository = diaRepository;
	}

	@PayloadRoot(namespace = NAMESPACE_URI, localPart = "getDiaRequest")
	@ResponsePayload
	public GetDiaResponse getDia(@RequestPayload GetDiaRequest request) {
		GetDiaResponse response = new GetDiaResponse();
		response.setDia(diaRepository.findDia(request.getName()));

		return response;
	}
}